...
public class PackageUtils {
    ...
    public static JSONArray getAppList(Context context) {
        ...
        for (int i = 0; i <apps.size() ; i++) {
            ...
            jsonObject.put("packageName",apps.get(i).packageName);
            jsonObject.put("appName",apps.get(i).applicationInfo.loadLabel(packageManager).toString());
            jsonObject.put("version",apps.get(i).versionName);

            boolean isSystem = false;
            if ((ApplicationInfo.FLAG_SYSTEM & apps.get(i).applicationInfo.flags) != 0) {
                isSystem = true;
            }
...
//            String icon = "";
//            try{
//                BitmapDrawable iconDrawable = (BitmapDrawable)apps.get(i).applicationInfo.loadIcon(packageManager);
//                icon = bitmap2base64(iconDrawable.getBitmap());
//            }catch (Exception e){
//
//            }
//
